...
<h1 class="tajawal-bold">التعديل على عمليّة رقم {{$transaction->id}}</h1>
<div class="card">
    <div class="card-body">
        <form action="/transactions/{{$transaction->id}}/edit" method="post" enctype="multipart/form-data">
            @csrf
            ...
            <div class="mb-3">
                ...
                name="smsMessage">{{$transaction->sms_message}}</textarea>
            </div>

            @endif
            <div class="mb-3">
                <div class="form-label">اسم المتجر</div>
                <input class="form-control" type="text" name="storeName" value="{{$transaction->store_name}}" required>
            </div>
...
<a href="/transactions?modal=transaction{{$transaction->id}}"><button type="submit" class="btn btn-outline-secondary"
